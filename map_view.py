...
COUNTRY_MAP = {
    "Venemaa": ("RUS", "Venemaa"), "Soome": ("FIN", "Soome"),
    "Läti": ("LVA", "Läti"), "Rootsi": ("SWE", "Rootsi"),
    "Ungari": ("HUN", "Ungari"), "Ukraina": ("UKR", "Ukraina"),
    "Moldova": ("MDA", "Moldova"), "Eesti": ("EST", "Eesti"),
    "Norra": ("NOR", "Norra"),
}
...
def norm_name(value):
    ...
    text = re.sub(r"\b(republic|oblast|krai|kray|autonomous|okrug|district|raion|rayon|municipal|region|of|the)\b", " ", text)
    ...
    for data_name in sorted(set(x for x in data_names if pd.notna(x))):
        ...
        for candidate in [data_name] + aliases.get(data_name, []):
            result = process.extractOne(
                norm_name(candidate), normalized,
...
def render_country_map(df):
    ...
    if counts.empty:
        ...
        return
    fig = px.choropleth(
        counts, locations="country_iso3", color="museaale",
        hover_name="country_clean", projection="natural earth",
    ...
    st.dataframe(counts.sort_values("museaale", ascending=False), hide_index=True, use_container_width=True)
...
def render_regions_map(df):
    ...
    if missing:
        st.warning("Regioonikaardi jaoks puuduvad veerud: " + ", ".join(sorted(missing)))
        return
    rus = df[(df["country"] == "Venemaa") & df["modern_region_eng"].notna()].copy()
    if rus.empty:
        st.info("Praeguse filtriga ei ole Venemaa regioonide andmeid.")
        return
    adm1 = load_boundaries(ADM1_URL)
    name_col = best_name_column(adm1)
    counts = rus.groupby(["modern_region_est", "modern_region_eng"], as_index=False).agg(museaale=("object_id", "nunique"))
    matches = match_table(counts["modern_region_eng"], adm1[name_col].dropna().astype(str).tolist(), REGION_ALIASES)
    counts = counts.merge(matches, left_on="modern_region_eng", right_on="data_name", how="left")
    map_df = adm1.merge(counts, left_on=name_col, right_on="boundary_name", how="inner").reset_index(drop=True)
    if map_df.empty:
    ...
    fig = px.choropleth_map(
        map_df, geojson=geojson, locations="map_id",
        featureidkey="properties.map_id", color="museaale",
        hover_name="modern_region_est",
        hover_data={"modern_region_eng": True, "match_score": True, "map_id": False},
        map_style="carto-positron", center={"lat": 61, "lon": 65},
        zoom=2.2, opacity=0.75, title="Museaalide arv Venemaa regiooniti",
    )
    fig.update_layout(margin=dict(l=0, r=0, t=45, b=0))
    st.plotly_chart(fig, use_container_width=True)
    st.dataframe(counts.sort_values("museaale", ascending=False), hide_index=True, use_container_width=True)
...
def render_districts_map(df):
    required = {"country", "modern_region_est", "modern_region_eng", "modern_rajon_est", "modern_rajon_eng", "object_id"}
    missing = required - set(df.columns)
    if missing:
        st.warning("Rajoonikaardi jaoks puuduvad veerud: " + ", ".join(sorted(missing)))
        return
    rus = df[(df["country"] == "Venemaa") & df["modern_region_eng"].notna() & df["modern_rajon_eng"].notna()].copy()
    if rus.empty:
        st.info("Praeguse filtriga ei ole Venemaa rajoonide andmeid.")
        return
    regions = sorted(rus["modern_region_eng"].dropna().unique())
    selected = st.selectbox("Vali regioon", regions, key="map_selected_region")
    adm1 = load_boundaries(ADM1_URL)
    adm2 = load_boundaries(ADM2_URL)
    adm1_name = best_name_column(adm1)
    adm2_name = best_name_column(adm2)
    points = adm2[[adm2_name, "geometry"]].copy()
    points["geometry"] = points.geometry.representative_point()
    parents = gpd.sjoin(points, adm1[[adm1_name, "geometry"]], how="left", predicate="within")[[adm2_name, adm1_name]].drop_duplicates()
    adm2 = adm2.merge(parents, on=adm2_name, how="left")
    region_match = match_table([selected], adm1[adm1_name].dropna().astype(str).tolist(), REGION_ALIASES)
    if region_match.empty or pd.isna(region_match.iloc[0]["boundary_name"]):
        st.warning("Valitud regioon ei leidnud piirifailis vastet.")
        return
    adm2_region = adm2[adm2[adm1_name] == region_match.iloc[0]["boundary_name"]].copy()
    counts = (
        rus[rus["modern_region_eng"] == selected]
        .groupby(["modern_region_est", "modern_region_eng", "modern_rajon_est", "modern_rajon_eng"], as_index=False)
        .agg(museaale=("object_id", "nunique"))
    )
    matches = match_table(counts["modern_rajon_eng"], adm2_region[adm2_name].dropna().astype(str).tolist(), threshold=62)
    counts = counts.merge(matches, left_on="modern_rajon_eng", right_on="data_name", how="left")
    map_df = adm2_region.merge(counts, left_on=adm2_name, right_on="boundary_name", how="inner").reset_index(drop=True)
    if map_df.empty:
        st.warning("Selle regiooni rajoonid ei leidnud piirifailis vastet.")
        st.dataframe(counts, hide_index=True, use_container_width=True)
        return
    map_df["map_id"] = map_df.index.astype(str)
    center = map_df.geometry.union_all().centroid
    fig = px.choropleth_map(
        map_df, geojson=map_df.__geo_interface__, locations="map_id",
        featureidkey="properties.map_id", color="museaale",
        hover_name="modern_rajon_est",
        hover_data={"modern_rajon_eng": True, "modern_region_est": True, "match_score": True, "map_id": False},
        map_style="carto-positron", center={"lat": center.y, "lon": center.x},
        zoom=4.2, opacity=0.78, title=f"Rajoonid: {selected}",
    )
    fig.update_layout(margin=dict(l=0, r=0, t=45, b=0))
    st.plotly_chart(fig, use_container_width=True)
    st.dataframe(counts.sort_values("museaale", ascending=False), hide_index=True, use_container_width=True)
...
def render_control(df):
    ...
    c2.metric("Tuvastatud riigiga", df[df["country_iso3"].notna()]["object_id"].nunique())
    if "modern_region_eng" in df.columns:
        n_regions = df[(df["country"] == "Venemaa") & df["modern_region_eng"].notna()]["object_id"].nunique()
    else:
        n_regions = 0
...
def render_map(filtered_df):
    ...
    st.caption("Kaart kasutab haldusüksuste polügoone. Arv näitab unikaalsete museaalide hulka.")
    if filtered_df is None or filtered_df.empty:
        st.info("Praeguse filtriga ei ole kaardil museaale.")
        return
    df = prepare_map_data(filtered_df)
    tab1, tab2, tab3, tab4 = st.tabs(["Riigid", "Venemaa regioonid", "Venemaa rajoonid", "Vastete kontroll"])
    with tab1:
